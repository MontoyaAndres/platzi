...
app.get("/login", function(req, res) {
  ...
  const queryString = querystring.stringify({
    ...
    state // to verify if the state is the same when spotify sends the /callback (this is for avoiding xss attacks)
  });
...
app.get("/callback", function(req, res, next) {
  ...
  // verify if the state returned by spotify is the same from the cookie `auth_state`
  if (state === null || state !== auth_state) {
    next(new Error("The state does not match"));
  }
